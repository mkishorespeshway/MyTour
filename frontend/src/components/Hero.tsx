...
const Hero = () => {
  ...
  const [title, setTitle] = useState("Discover Your Next")
    ...
    (async () => {
      try {
        const { data, error } = await supabase
          .from("home_content")
          .select<{ title: string; subtitle: string; image_url: string }>("*")
          .eq("section", "hero")
          .maybeSingle()
        if (error) return
        const item = data as { title: string; subtitle: string; image_url: string } | null
        if (item) {
          setTitle(item.title || "Discover Your Next")
          setSubtitle(item.subtitle || "Explore breathtaking destinations, plan unforgettable trips, and create memories that last a lifetime")
          setBg(item.image_url || null)
        }
      } catch {
        // keep defaults
      }
